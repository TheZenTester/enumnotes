...
# This is a reference to the an old website from 2010, there likely are tools missing/out of date. Info provided for reference.
...
- `host`
    - `host [-aCdlnrTwv ] [-c class ] [-N ndots ] [-R number ] [-t type ] [-W wait ] name [server ] -v verbose format -t (query type)` 
    - Allows a user to specify a record type i.e. A, NS, or PTR. -a Same as –t ANY. -l Zone transfer (if allowed). -f Save to a specified filename.
- `nslookup`
